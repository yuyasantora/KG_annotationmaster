...
class dragBBox:
    ...
    @staticmethod
    def handler_left_mouse_up(x, y):
        """ドラッグ（リサイズ）終了時に状態をリセットする関数をここに追加します。"""
...
def save_bounding_box(annotation_paths, class_index, point_1, point_2, width, height, current_image_full_path=None):
    for ann_path in annotation_paths:
        if '.txt' in ann_path:
            ...
            os.makedirs(os.path.dirname(ann_path), exist_ok=True)
            if not os.path.exists(ann_path) and current_image_full_path:
                abs_path = os.path.abspath(current_image_full_path)
                folder_name_for_xml = "PASCAL_VOC"
                image_name = os.path.basename(current_image_full_path)
                create_PASCAL_VOC_xml(ann_path, abs_path, folder_name_for_xml, image_name, str(height), str(width), '3')
            elif not os.path.exists(ann_path) and not current_image_full_path:
                print(f"警告: {ann_path} に対応する画像パスが不明です。空のXMLを作成できません。")
...
if __name__ == '__main__':
    ...
    if args.image:
        ...
        if os.path.isfile(image_path):
            ...
            if test_img is not None:
                ...
                # 単一画像の場合は同じ画像を2回追加してスライダーを有効にする
                IMAGE_PATH_LIST.append(image_path)  # 同じ画像を2回追加
    ...
    # スライダーの最大値が0以下の場合の処理 (単一画像モード)
    if last_img_index <= 0:
        print("警告: 単一画像モードで実行します（スライダー無効）")
        # スライダーは作成しないが、ウィンドウは作成する
        cv2.namedWindow(WINDOW_NAME, cv2.WINDOW_KEEPRATIO)
        cv2.resizeWindow(WINDOW_NAME, 1000, 700) # 必要であればサイズ調整
        cv2.setMouseCallback(WINDOW_NAME, mouse_listener) # マウスコールバックは設定
        # cv2.createTrackbar は作成しない
    else:
        # create window with slider (複数画像モード)
        cv2.namedWindow(WINDOW_NAME, cv2.WINDOW_KEEPRATIO)
    ...
    while True:
        ...
        if point_1[0] != -1:
            ...
            if point_2[0] != -1:
                ...
                current_image_full_path = IMAGE_PATH_LIST[img_index]
                save_bounding_box(annotation_paths, class_index, point_1, point_2, width, height, current_image_full_path)
                # reset the points
                point_1 = (-1, -1)
                point_2 = (-1, -1)
